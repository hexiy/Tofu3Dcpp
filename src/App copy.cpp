...
int main() {
    ...
    if (!shaderProgram->compileShaderFromFile("src/shaders/triangle.vs", GL_VERTEX_SHADER)) {
        printf("Vertex shader failed to compile!\n%s", shaderProgram->log().c_str());
        exit(1);
    }

    if (!shaderProgram->compileShaderFromFile("src/shaders/triangle.fs", GL_FRAGMENT_SHADER)) {
    ...
    projection_matrix = glm::perspective(45.0f, (GLfloat) DEFAULT_WINDOW_WIDTH / (GLfloat) DEFAULT_WINDOW_HEIGHT, 0.1f,
                                         100.0f);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        shaderProgram->setUniform("frag_color",
                                  glm::vec3((float) sin(now), 1.0f + (float) atan(now), (float) cos(now)));
