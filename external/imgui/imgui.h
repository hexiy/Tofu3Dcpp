...
namespace ImGui
{
    ...
    // - In this header file we use the "label"/"Id" terminology to denote a string that will be displayed + used as an ID,
    ...
    // - Those are merely shortcut to calling Text() with a format string. Output single value in "Id: value" format (tip: freely declare more in your code to handle your types. you can add functions to the ImGui namespace)
    ...
    //  - They are convenient to easily create context menus, hence the Id.
    ...
    // - 2. Optionally call TableSetupColumn() to submit column Id/flags/defaults.
    ...
    IMGUI_API const char*           TableGetColumnName(int column_n = -1);      // return "" if column didn't have a Id declared by TableSetupColumn(). Pass -1 to use current column.
    ...
    IMGUI_API void          SetTabItemClosed(const char* tab_or_docked_window_label);           // notify TabBar or Docking system of a closed tab/window ahead (useful to reduce visual flicker on reorderable tab bars). For tab-bar: call after BeginTabBar() and before Tab submissions. Otherwise call with a window Id.
    ...
    IMGUI_API const char*   GetKeyName(ImGuiKey key);                                           // [DEBUG] returns English Id of the key. Those names a provided for debugging purpose and are not meant to be saved persistently not compared.
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_AlwaysInsertMode    = ImGuiInputTextFlags_AlwaysOverwrite   // [renamed in 1.82] Id was not matching behavior
#endif
};
...
enum ImGuiStyleVar_
{
    // Enum Id --------------------- // Member in ImGuiStyle structure (see ImGuiStyle for descriptions)
...
struct ImGuiIO
{
    ...
    // Optional: Platform/Renderer backend Id (informational only! will be displayed in About Window) + User data for backend/wrappers to store their own stuff.
...
// Callbacks (follow a flag Id and see comments in ImGuiInputTextFlags_ declarations for more details)
...
struct ImDrawList
{
    ...
    const char*             _OwnerName;         // Pointer to owner window's Id for debugging
...
namespace ImGui
{
    ...
    static inline void  CaptureKeyboardFromApp(bool want_capture_keyboard = true)           { SetNextFrameWantCaptureKeyboard(want_capture_keyboard); } // Renamed as Id was misleading + removed default value.
    static inline void  CaptureMouseFromApp(bool want_capture_mouse = true)                 { SetNextFrameWantCaptureMouse(want_capture_mouse); }       // Renamed as Id was misleading + removed default value.
    ...
    //IMGUI_API bool      Begin(char* Id, bool* p_open, ImVec2 size_first_use, float bg_alpha = -1.0f, ImGuiWindowFlags flags=0); // OBSOLETED in 1.52 (between Aug 2017 and Oct 2017): Equivalent of using SetNextWindowSize(size, ImGuiCond_FirstUseEver) and SetNextWindowBgAlpha().
...
#error IMGUI_DISABLE_METRICS_WINDOW was renamed to IMGUI_DISABLE_DEBUG_TOOLS, please use new Id.
#endif
